...
fun main(args:Array<String>){
    ...
    val fileName:String = "src/test/testtext.txt"
    val content = File(fileName).readBytes()

    val headNode:Node = algo.makeCode(process.makeTable(content))

    val codeMap:MutableMap<Int,String> =algo.getCodeMap(headNode)
    var str:String = ""
    ...
    File("src/test/test.grml").writeBytes(bittest.toByteArray())
    codeMap.forEach{println(it)}
    val loadName:String = "src/test/test.grml"
    ...
    File("src/test/testoutput.txt").writeBytes(dec.decompress(sBuild2.toString(),headNode,offset,content.size))
